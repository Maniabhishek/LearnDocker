...
- now again if we want to export another variable ```echo APPNAME=google >> .bashrc``` and if we try ```echo $APPNAME``` we will not see anything , we need either restart the session or refresh the bashrc file using ```source .bashrc``` for this command you have to be in root directory or use ```source ~/.bashrc``` from any director ~ means home directory 
